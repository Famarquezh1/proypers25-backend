...
function render(summary,live,q){var opens=liveManaged(live);var allocation=summary.allocation||{};var account=summary.account||summary.binance_account||{};var assets=summary.assets||summary.holdings||[];var recent=summary.recent_trades||[];var equity=num(account.total_equity_usdt||summary.total_equity_usdt||summary.account_total_usdt);var usdt=num(account.usdt_balance||summary.usdt_balance||summary.available_usdt);var exposure=num(allocation.api_exposure_usdt||allocation.managed_capital_usdt);var unrealized=opens.reduce(function(a,p){return a+num(p.unrealized_pnl_usdt)},0);quick.innerHTML=card('Patrimonio total',money(equity),'Cuenta completa','wide')+card('USDT disponible',money(usdt),'Saldo disponible')+card('Capital máximo administrable',money(allocation.configured_max_total_usdt||0),'Límite configurado')+card('Capacidad libre',money(Math.max(0,num(allocation.configured_max_total_usdt)-exposure)),opens.length+' adquisiciones')+card('Ganancia / pérdida',money(unrealized),opens.length?'PnL abierto':'Sin posiciones')+card('Estado operativo',live.exit_engine_healthy===false?'REVISAR':'OPERATIVO',live.spot_only===false?'Spot only no confirmado':'Spot only');var st=q.status||{};var latest=q.latest||{};var exact=latest.qubo||{};var inspired=latest.quantum_inspired||{};var samples=q.sample_counts||{};var th=st.thresholds||{};qubo.innerHTML=card('Estado',st.promotion_state||st.mode||q.mode||'SHADOW','No ejecuta órdenes','wide')+card('Muestras',String(st.settled_samples||samples.settled||0)+' / '+String(th.min_settled||30),'Cerradas / requeridas')+card('Win Rate',pct(num(st.win_rate_vs_greedy)*100),'Gate mínimo '+pct(num(th.min_win_rate)*100))+card('Ventaja media',pct(st.mean_excess_vs_greedy_pct),'QUBO vs greedy')+card('QUBO exacto',(exact.symbols||[]).join(', ')||'—',money(exact.capital_usdt)+' asignados','wide')+card('Quantum-inspired',(inspired.symbols||[]).join(', ')||'—',money(inspired.capital_usdt)+' asignados','wide');positions.innerHTML=table(['Activo','Fecha compra','Precio compra','Precio actual','Variación','PnL US$','Take Profit','Stop Loss','Modo'],opens.map(function(p){var cp=p.current_price||p.current_price_usdt||p.mark_price;var ep=p.entry_price;var gp=ep&&cp?((num(cp)/num(ep)-1)*100):null;return ['<b>'+esc(p.symbol)+'</b>',esc(p.opened_at||p.created_at||'—'),esc(money(ep)),esc(money(cp)),esc(pct(gp)),esc(money(p.unrealized_pnl_usdt)),esc(money(p.effective_tp_price||p.take_profit_price)),esc(money(p.effective_sl_price||p.stop_loss_price)),esc(p.entry_mode||'—')]}));var blockers=(summary.engine_diagnosis&&summary.engine_diagnosis.blockers)||[];health.innerHTML=card('Exit Engine',live.exit_engine_healthy===false?'ERROR':'OK',live.exit_engine_healthy===false?'Revisar':'Monitoreando',live.exit_engine_healthy===false?'bad':'ok')+card('Spot only',live.spot_only===false?'NO':'SÍ','Sin futuros/margen')+card('Bloqueadores',String(blockers.length),blockers.slice(0,2).join(' · ')||'Sin bloqueadores')+card('Profit Factor',summary.profit_factor||'—','Histórico del bot')+card('QUBO real execution',q.real_execution_enabled?'ACTIVO':'DESACTIVADO','Debe permanecer desactivado en shadow');holdings.innerHTML=table(['Activo','Cantidad','Valor USDT','Clasificación','Precio promedio'],(Array.isArray(assets)?assets:[]).map(function(a){var klass=a.dust_residual?'Residuo de operación / Dust':(a.holding_class||'—');return ['<b>'+esc(a.asset||a.symbol)+'</b>',esc(a.quantity),esc(money(a.value_usdt)),esc(klass),esc(money(a.average_price||a.average_cost_usdt))]}));activity.innerHTML=table(['Activo','Resultado','PnL','Fecha'],recent.slice(0,20).map(function(t){return ['<b>'+esc(t.symbol||t.asset)+'</b>',esc(t.action||t.status||t.result||'—'),esc(money(t.net_pnl_usdt||t.pnl_usdt)),esc(t.closed_at||t.created_at||t.timestamp||'—')]}))}
async function load(){var s=secretInput.value.trim();if(!s){message.textContent='Ingresa la clave privada.';content.classList.add('hidden');return}try{localStorage.setItem('proypers25_summary_secret',s)}catch(e){}message.textContent='Actualizando datos…';refreshBtn.disabled=true;try{var headers={'x-investments-secret':s};var all=await Promise.all([getJson('/internal/investments/summary',headers),getJson('/internal/spot-live/evidence',headers),getJson('/internal/investments/qubo-status',headers)]);render(all[0],all[1],all[2]);content.classList.remove('hidden');message.textContent=''}catch(e){content.classList.add('hidden');message.textContent=e.message||'No fue posible cargar el dashboard.'}finally{refreshBtn.disabled=false}}
refreshBtn.addEventListener('click',load);secretInput.addEventListener('keydown',function(e){if(e.key==='Enter')load()});try{var saved=localStorage.getItem('proypers25_summary_secret');if(saved)secretInput.value=saved}catch(e){}
})();
</script>
</body></html>`);
});
