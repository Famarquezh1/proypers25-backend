...
function positionView(position, price, cycle) {
  ...
  return {
    ...
    entry_mode: position.entry_mode || null,
...
async function buildLiveEvidence() {
  ...
  const positionViews = positions
    .map((position) => positionView(position, prices[position.symbol], cycle))
    .sort((a, b) => new Date(b.opened_at || 0) - new Date(a.opened_at || 0));
  const latestPosition = positionViews[0] || null;
  const latestCycle = cycle ? {
    ...
    gates: cycle.gates || {},
    exit_diagnostics: cycle.exit_diagnostics || null,
    dust_cleanup: cycle.dust_cleanup || null
  } : null;
  ...
  return {
    ...
    // Backwards-compatible single item plus the complete managed Spot set.
    open_position: latestPosition,
    open_positions: positionViews,
    managed_spot_acquisitions: positionViews,
    managed_spot_acquisition_count: positionViews.length,
    engine: {
