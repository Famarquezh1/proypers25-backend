...
const { runRealSpotPreflightCheck } = require('../services/binanceSpotRealExecutor');
...
router.post('/internal/cron/velas/prealerts', async(req, res) => {
    ...
    setImmediate(async() => {
        try {
            ...
            if (summary?.skipped) console.warn('[CRON] prealert cycle skipped', summary);
        } catch (err) {
...
router.post('/internal/cron/velas/full-cycle', async(req, res) => {
    if (!checkSecret(req, res)) return;
    res.status(200).json({ ok: true, source: 'scheduler', job: 'velas-full-cycle' });
    setImmediate(async() => {
        try {
            await runPredictionCycle();
            await runVerificationCycle();
...
router.post('/internal/cron/binance/spot-real-execution-dryrun', async(req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ ok: false, error: error.message, type: 'DRY_RUN_FAILED' });
    }
});
...
router.post('/internal/cron/binance/spot-real-execution-approve', async(req, res) => {
    ...
    try {
        ...
        if (!dryRunSnap.exists) return res.status(404).json({ ok: false, error: 'Dry-run not found', dry_run_id });
        const dryRun = dryRunSnap.data();
        if (new Date() > new Date(dryRun.expires_at)) {
            return res.status(410).json({ ok: false, error: 'Dry-run has expired', expired_at: dryRun.expires_at });
        }
        ...
        return res.status(500).json({ ok: false, error: error.message, type: 'APPROVAL_FAILED' });
    }
});
...
router.post('/cron/binance/spot-real-preflight', (req, res) => {
    ...
    (async() => {
        try {
            ...
            return res.json({ ok: preflight.ok, preflight, cron_executed: true, blocked: false });
        } catch (error) {
            console.error('[CRON_PREFLIGHT] Error:', error.message);
            return res.status(500).json({ ok: false, error: error.message, cron_executed: false });
        }
    })();
});
