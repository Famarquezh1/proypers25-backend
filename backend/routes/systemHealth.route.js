...
async function buildHealth() {
  const [control, adaptive, cost, quantRun, adaptiveRun, discoveryRun, costRun, openPositions] = await Promise.all([
  ...
  const components = [
    ...
    },
    {
      ...
      updated_at: control?.reconciliation_gate_released_at || control?.updated_at || null
    },
    {
      id: 'discovery',
      label: 'Descubrimiento de activos',
    ...
    {
      id: 'quant',
      label: 'Laboratorio cuantitativo',
...
function render(d){summary.innerHTML=[['Estado general',d.overall,cls(d.overall)],['Componentes sanos',d.summary.green,'green'],['Advertencias',d.summary.yellow,'yellow'],['Bloqueos',d.summary.red,'red'],['Posiciones abiertas',d.open_positions,''],['Límite por posición','US$ '+Number(d.limits.max_position_usdt||0).toFixed(2),'']].map(x=>'<div class="card"><div class="label">'+x[0]+'</div><div class="value '+x[2]+'">'+x[1]+'</div></div>').join('');components.hidden=false;components.innerHTML=d.components.map(c=>'<div class="row"><div class="wide"><b>'+c.label+'</b></div><div class="'+cls(c.state)+'"><span class="dot"></span><b>'+c.state+'</b></div><div>'+c.detail+'</div><div class="muted">'+date(c.updated_at)+'</div></div>').join('');}
