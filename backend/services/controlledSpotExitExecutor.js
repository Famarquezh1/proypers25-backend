...
async function claimPosition(db, ref, reason, price) {
  return db.runTransaction(async (tx) => {
    ...
    tx.update(ref, {
      exit_status: 'EXIT_PENDING', exit_claim_id: claimId,
      exit_reason_pending: reason, exit_price_observed: price,
      exit_claimed_at: new Date().toISOString()
    });
    return { id: snap.id, ...data, claimId };
  });
}
...
async function executeMarketSell(symbol, quantity) {
  return privateRequest('POST', '/api/v3/order', { symbol, side: 'SELL', type: 'MARKET', quantity });
}
...
async function finalizeExit(db, ref, position, order, reason, observedPrice) {
  ...
  if (!(executedQty > 0)) throw new Error('SELL_ORDER_WITHOUT_EXECUTION');
  ...
  await db.runTransaction(async (tx) => {
    ...
    const [latest, balanceSnap] = await Promise.all([tx.get(ref), tx.get(balanceRef)]);
    ...
    tx.update(ref, fullyClosed ? {
      status: 'REAL_CLOSED', exit_status: 'EXIT_FILLED', closing_reason: reason,
      closed_at: closedAt, exit_price: exitPrice, pnl_usdt: netPnl,
      sell_order_id: order.orderId, sold_quantity: executedQty,
      ...
      exit_status: 'PARTIAL_EXIT_FILLED', last_partial_exit_at: closedAt,
      last_sell_order_id: order.orderId, last_sold_quantity: executedQty
    });

    tx.set(db.collection(RESULTS).doc(resultId), {
      id: resultId, position_id: position.id, symbol: position.symbol,
      entry_price: Number(position.entry_price || 0), exit_price: exitPrice,
      quantity: executedQty, allocated_capital_usdt: allocatedCapital,
      quote_received_usdt: quoteReceived, gross_pnl_usdt: grossPnl,
      actual_fee_usdt: feeUsdt, net_pnl_usdt: netPnl, net_pnl_pct: netPnlPct,
      closing_reason: reason, opened_at: position.opened_at || null,
      closed_at: closedAt, order_id: order.orderId, order_status: order.status,
      fully_closed: fullyClosed, real_mode: true, safety_version: SAFETY_VERSION
    });
...
async function evaluateAndExecuteRealSpotExits(db, config, options = {}) {
  ...
  for (const doc of snapshot.docs) {
    ...
    try {
      ...
      try {
        ...
        const order = await executeMarketSell(symbol, quantity);
        outcomes.push({ symbol, action: 'SELL', reason, order_id: order.orderId,
  ...
  return {
    ok: true, blocked: false, evaluated: snapshot.size,
    ...
    outcomes, duration_ms: Date.now() - startedAt
  };
}

module.exports = { SAFETY_VERSION, assertExitConfig, determineExit, floorToStep, evaluateAndExecuteRealSpotExits };
