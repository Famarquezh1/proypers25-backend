...
function blockerDetails({ reconciliation = {}, exits = {}, adaptiveGate = {}, promotionGate = {}, paperGate = {}, autonomy = {}, config = {}, safetyFailures = [], startedAt }) {
  if (Array.isArray(safetyFailures) && safetyFailures.length) {
    return safetyFailures.map((failure) => ({
      component: failure.component || 'Unknown',
      reason: failure.code || failure.condition || 'UNKNOWN_BLOCKER',
      missing_condition: failure.expected || 'Required safety condition must pass',
      actual: failure.actual ?? null,
      since: startedAt || nowIso()
    }));
  }
  ...
  push('Paper → Real', paperGate.allowed !== true, text(...(paperGate.reasons || []), paperGate.reason, 'PAPER_REAL_ENTRY_GATE_BLOCKED'),
    text(paperGate.missing_condition, 'La oportunidad actual debe superar evidencia Paper y confirmación técnica'), paperGate.updated_at || paperGate.created_at);
...
async function persistCycleEvidence(db, input) {
  ...
  const cycle = plain({
    ...
    promotion_confidence: input.promotionGate || null,
  ...
  await Promise.all([
    ...
    persistActivity(db, { event_type: 'DISCOVERY', source: 'BINANCE_PUBLIC', created_at: completedAt, cycle_id: id, result: input.discovery?.scan_id ? 'COMPLETED' : 'MISSING', details: input.discovery || null })
  ]);
