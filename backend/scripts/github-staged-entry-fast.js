...
const baseline={net:+k[H][4]/p0-1-COST,deployed:1};
const variants=[];
for(const m of [1,2,3]){const c=+k[m][4],hi=+k[m][2],lo=+k[m][3],op=+k[m][1];const ret=c/p0-1,closePos=(c-lo)/Math.max(1e-12,hi-lo),adverse=lo/p0-1;const accept=ret>0&&closePos>=.7&&adverse>-.01;const entry=+k[m+1][1];const exit=+k[Math.min(m+1+H,k.length-1)][4]; const seed=.25,seedNet=exit/p0-1-COST,addNet=exit/entry-1-COST; variants.push({m,accept,net:accept?(seed*seedNet+(1-seed)*addNet):seed*seedNet,deployed:accept?1:seed,ret,closePos,adverse})}
...
const out={ok:true,research_only:true,rows:rows.length,rule:'25% immediate seed; if confirmation passes after completed minute, add remaining 75% next minute; otherwise retain only 25% seed; same 4h horizon',cost:COST,baseline:{train:stats(train.map(r=>r.baseline)),test:stats(test.map(r=>r.baseline))},variants:{}};
