...
async function fetchJson(url){
  ...
  try{const r=await fetch(url,{signal:c.signal,headers:{'user-agent':'proypers25-v8-hunter-shadow-model/1.0'}});if(!r.ok)throw new Error(`HTTP ${r.status}`);return await r.json();}
...
async function universe(){
  ...
  if(!ranked.includes('BTCUSDT')) ranked.push('BTCUSDT');
  return ranked;
}
...
function trainLogistic(rows){
  ...
  return {mu:s.mu,sd:s.sd,w,predict(x){x=norm(x,s);let z=w[0];for(let j=0;j<d;j++)z+=w[j+1]*x[j];return sigmoid(z);}};
}
...
async function main(){
  const output=process.argv[2]||'spot-v8-hunter-shadow-model.json';
  const now=Date.now(),trainEnd=utcDay(now),trainStart=trainEnd-TRAIN_DAYS*DAY_MS,fitCut=trainEnd-2*DAY_MS;
  const dataStart=trainStart-DAY_MS-WARM*STEP_MS;
  const symbols=await universe();
  const loaded=await mapLimit(symbols,8,async symbol=>({symbol,rows:await klines(symbol,dataStart,trainEnd)}));
  ...
  for(const [symbol,rows] of data){
    ...
    for(let i=WARM;i<rows.length-HOLD_BARS-2;i++){
  ...
  if(fit.length<300||cal.length<80)throw new Error(`insufficient V8 hunter samples fit=${fit.length} cal=${cal.length}`);
  const model=trainLogistic(fit),choice=chooseThreshold(cal,model);
  if(!choice)throw new Error('no V8 hunter threshold with enough calibration trades');
  const payload={
    version:'V8_HUNTER_SHADOW_MODEL_V1',mode:'SHADOW_MODEL_ONLY',decisionInfluence:false,productionAction:'NONE',
    generatedAt:new Date().toISOString(),training:{trainDays:TRAIN_DAYS,fitSamples:fit.length,calSamples:cal.length,decisionCadenceMinutes:5,takeProfit:TP,stopLoss:SL,holdMinutes:HOLD_BARS*5,roundtripCost:COST},
    universe:{requested:MAX_SYMBOLS,loaded:data.size},threshold:choice.threshold,
    calibration:{trades:choice.metrics.trades,winRate:choice.metrics.winRate,avgNet:choice.metrics.avgNet,compoundedGrowth:choice.metrics.compoundedGrowth,maxDrawdown:choice.metrics.maxDrawdown},
    model:{featureNames:FEATURE_NAMES,mu:model.mu,sd:model.sd,w:model.w}
  };
  fs.writeFileSync(output,JSON.stringify(payload,null,2));
  console.log(JSON.stringify({ok:true,output,version:payload.version,threshold:payload.threshold,fitSamples:fit.length,calSamples:cal.length,calibration:payload.calibration}));
}
