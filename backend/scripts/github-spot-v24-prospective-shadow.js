...
async function bars(s,limit=290,endTime=null){const tail=endTime?'&endTime='+encodeURIComponent(endTime):'';const r=await get(BASE+'/api/v3/klines?symbol='+encodeURIComponent(s)+'&interval=5m&limit='+limit+tail);return r.map(x=>({t:+x[0],h:+x[2],l:+x[3],c:+x[4],q:+x[7],n:+x[8],ct:+x[6]}))}
...
async function main(){const now=Date.now(),state=load(),e={generated_at:iso(now),shadow_only:true,no_order_created:true,production_action:'NONE',new_signals:[],new_outcomes:[],errors:[]};state.signals=state.signals||[];state.outcomes=state.outcomes||[];
 for(const s of state.signals.filter(x=>!x.resolved_at&&now-x.signal_time>=HORIZON_MS)){try{const b=await bars(s.symbol,110,s.signal_time+HORIZON_MS);const after=b.filter(x=>x.t>s.signal_time&&x.t<=s.signal_time+HORIZON_MS);if(after.length<90){e.errors.push({stage:'RESOLVE_WINDOW_INCOMPLETE',symbol:s.symbol,id:s.id,bars:after.length});continue;}const max=Math.max(...after.map(x=>x.h))/s.signal_price-1,min=Math.min(...after.map(x=>x.l))/s.signal_price-1,o={id:'outcome_'+s.id,symbol:s.symbol,signal_at:s.signal_at,resolved_at:iso(now),max_return_8h:max,min_return_8h:min,exploded:max>=EXPLOSION,shadow_only:true,no_order_created:true};s.resolved_at=o.resolved_at;state.outcomes.push(o);e.new_outcomes.push(o)}catch(err){e.errors.push({stage:'RESOLVE',symbol:s.symbol,error:err.message})}}
