...
const TIMING_KEEPS=[.35,.45,.55,.65,.75,.85,1];

function loadBase(){let src=fs.readFileSync(SOURCE,'utf8').replace(/\nmain\(\)\.catch[\s\S]*$/,'');src+=';globalThis.__me={loadR7,buildRaw,selectSignals};';const c=vm.createContext({require,console,process,fetch,URL,URLSearchParams,AbortController,Buffer,setTimeout,clearTimeout,__dirname,__filename:SOURCE});vm.runInContext(src,c,{filename:SOURCE});return c.__me}
...
function timingVec(s){
 ...
 const b=a[i],p1=a[i-1],p2=a[i-2],p3=a[i-3];
 ...
 const rangeNow=(Number(b.h)-Number(b.l))/Math.max(1e-12,Number(b.o));
 const rangeExpansion=rangeNow/Math.max(1e-9,avg(ranges));
 ...
 const compression=r30/Math.max(1e-9,r120);
 const ph=Math.max(...a.slice(i-12,i).map(x=>Number(x.h||0))),dist=ph>0?Number(b.c)/ph-1:0;
 let age=0;
 for(let k=i;k>=Math.max(12,i-6);k--){
   const h=Math.max(...a.slice(k-12,k).map(x=>Number(x.h||0)));
   if(h>0&&Number(a[k].c)>h)age++;else break;
 }
...
function line(rows){const x=rows.map(r=>Number(r.s.outcome?.mfe12||0)),y=rows.map(r=>r.net),xm=avg(x),ym=avg(y),den=x.reduce((a,v)=>a+(v-xm)**2,0)||1,b=x.reduce((a,v,i)=>a+(v-xm)*(y[i]-ym),0)/den;return {a:ym-b*xm,b}}
function fitEntry(lib,sigs){const opp=fit(sigs.map(s=>({s,y:oppTarget(s)})),ENTRY.lambdaOpportunity,baseVec);const rr=sigs.map(s=>({s,net:tradeNet(lib,s)})),ln=line(rr),mon=fit(rr.map(x=>({s:x.s,y:x.net-(ln.a+ln.b*Number(x.s.outcome?.mfe12||0))})),ENTRY.lambdaMonetization,baseVec);return {opp,mon}}
...
function timingTarget(lib,s){const o=s.outcome||{},net=tradeNet(lib,s);return net+.10*Number(o.mfe12||0)+.004*(o.winner5?1:0)+.008*(o.winner10?1:0)}
...
async function main(){
 ...
 const configs=[];for(const lambda of TIMING_LAMBDAS)for(const keep of TIMING_KEEPS)configs.push({lambda,keep,folds:[],selected:[]});
 let start=initial;
 for(let round=1;round<=4&&start<ds.length;round++){
  ...
  const ep=fitEntry(lib,train),es=entryStats(ep,train),trainEntry=selectEntry(ep,es,train,train),valEntry=selectEntry(ep,es,train,val);
  const baseM=metrics(lib,val,all);
  for(const cfg of configs){
    const tm=fit(trainEntry.map(s=>({s,y:timingTarget(lib,s)})),cfg.lambda,timingVec);
    if(!tm){cfg.folds.push({round,selected:0,pass:false});continue}
    const trainScores=trainEntry.map(s=>pred(tm,s,timingVec)),th=qtl(trainScores,1-cfg.keep);
    const selected=valEntry.filter(s=>pred(tm,s,timingVec)>=th),m=metrics(lib,selected,all),d=delta(m,baseM);
    cfg.selected.push(...selected);cfg.folds.push({round,entry_selected:valEntry.length,selected:selected.length,threshold:th,delta:d,objective:obj(d),pass:selected.length>=4&&nonneg(d)});
  }
  start=end;
 }
 const walkAll=dev.filter(s=>s.t>=ds[initial].t),walkBase=metrics(lib,ds.slice(initial),walkAll);
 for(const cfg of configs){const m=metrics(lib,cfg.selected,walkAll),d=delta(m,walkBase);cfg.aggregate={selected:cfg.selected.length,metrics:m,delta:d,objective:obj(d),pass_folds:cfg.folds.filter(f=>f.pass).length};cfg.viable=cfg.aggregate.selected>=16&&cfg.aggregate.pass_folds>=3&&nonneg(d)&&m.economic.netGrowth>0&&m.economic.avgNetRet>0;delete cfg.selected}
 ...
 if(chosen){
  const ep=fitEntry(lib,ds),es=entryStats(ep,ds),trainEntry=selectEntry(ep,es,ds,ds),holdEntry=selectEntry(ep,es,ds,hs);
  const tm=fit(trainEntry.map(s=>({s,y:timingTarget(lib,s)})),chosen.lambda,timingVec),th=qtl(trainEntry.map(s=>pred(tm,s,timingVec)),1-chosen.keep);
  const selected=holdEntry.filter(s=>pred(tm,s,timingVec)>=th),bm=metrics(lib,hs,hold),m=metrics(lib,selected,hold),d=delta(m,bm);
  confirmation={entry_selected:holdEntry.length,selected:selected.length,threshold:th,baseline:bm,metrics:m,delta:d};
 ...
 const report={version:'MONETIZATION_EDGE_V3_TIMING_FRESHNESS',generated_at:new Date().toISOString(),research_only:true,production_mutation:false,
 objective:'Add a causal pre-entry 5m timing/freshness discriminator on top of the frozen V1 opportunity+monetization policy. Select timing lambda/keep only by Apr-Jun expanding walk-forward, then test once on untouched July.',
 entry_policy:ENTRY,timing_features:TIMING,universe:{pool:built.poolSize,loaded:built.loaded,candidate_rows:raw.length,dev_signals:ds.length,holdout_signals:hs.length},configs:configs.map(x=>({lambda:x.lambda,keep:x.keep,viable:x.viable,aggregate:x.aggregate,folds:x.folds})),selected:chosen?{lambda:chosen.lambda,keep:chosen.keep,aggregate:chosen.aggregate,folds:chosen.folds}:null,confirmation,attribution:attr,confirmation_pass:pass,
 decision:!chosen?{label:'TIMING_FRESHNESS_NOT_FOUND_IN_WALK_FORWARD',ready:false}:pass?{label:'TIMING_FRESHNESS_CONFIRMED_RESEARCH_ONLY',ready:false}:{label:'TIMING_FRESHNESS_FAILED_FRESH_HOLDOUT',ready:false}};
 fs.mkdirSync(path.dirname(OUTPUT),{recursive:true});fs.writeFileSync(OUTPUT,JSON.stringify(report,null,2));
 console.log(JSON.stringify({version:report.version,selected:report.selected,top_configs:configs.slice(0,5).map(x=>({lambda:x.lambda,keep:x.keep,viable:x.viable,aggregate:x.aggregate})),confirmation,top_timing:attr?.slice(0,8)||null,confirmation_pass:pass,decision:report.decision},null,2));
}
