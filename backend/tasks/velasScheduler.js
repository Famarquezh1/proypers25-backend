...
const { warmExchangeInfoCache, warmMarginLeverageCache } = require('../lib/binanceFuturesExecutor');
const { getBinanceBotConfig } = require('../lib/binanceBotConfig');
const {
    processImpulseSignals,
    runActivePositionSafetyControl
} = require('../services/impulseExecutionEngine');
...
const DEFAULT_PREDICTION_CONFIG = [
    { symbol: 'BTCUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'ETHUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'DOGEUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'HBARUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'SOLUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'ADAUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'XRPUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'BNBUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'AVAXUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'LINKUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'MATICUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'DOTUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'LTCUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'BCHUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'TRXUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'SHIBUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'TONUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'NEARUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'ATOMUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'ICPUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'XLMUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'OPUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'ARBUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'INJUSDT', timeframe: '5m', execution_mode: 'event_driven' },
    { symbol: 'APTUSDT', timeframe: '5m', execution_mode: 'event_driven' }
];
...
const SHADOW_VERIFICATION_QUERY_LIMIT = Math.max(
    100,
    Math.min(Number(process.env.SHADOW_VERIFICATION_QUERY_LIMIT || 300), 500)
);
const MAX_SHADOW_EVALUATIONS_PER_CYCLE = Math.max(
    1,
    Math.min(Number(process.env.MAX_SHADOW_EVALUATIONS_PER_CYCLE || 20), SHADOW_VERIFICATION_QUERY_LIMIT)
);
const MAX_VERIFICATION_RUNTIME_MS = Math.max(5000, Number(process.env.MAX_VERIFICATION_RUNTIME_MS || 25000));
const MAX_SHADOW_FETCH_CONCURRENCY = Math.max(
    1,
    Math.min(Number(process.env.MAX_SHADOW_FETCH_CONCURRENCY || 1), 3)
);
...
const PREALERT_MARGIN_LEVERAGE_WARM_SYMBOLS = Math.max(
    1,
    Math.min(ACTIVE_SYMBOLS.length, Number(process.env.PREALERT_MARGIN_LEVERAGE_WARM_SYMBOLS || 8))
);
...
async function runPredictionCycle(options = {}) {
    ...
    await mapWithConcurrency(predictionConfig, cycleConcurrency, async(config) => {
        ...
        try {
            ...
            const wouldEmitIfRelaxed = !result ?.suppression_reason &&
            ...
            if (PROFILING_FETCH_ENABLED && cycleType === 'prealert_cycle') {
                ...
                console.log('[PREALERT_SYMBOL_TIMING]', {
                    ...
                    post_prediction_gap_ms: pipelineTotalMs == null ? null : Math.max(0, symbolDurationMs - pipelineTotalMs),
        ...
        return await withTimeout(
            async() => {
                ...
                stageTimings.watchdogs_ms = logPrealertStageTiming(runId, 'watchdogs', watchdogStartedAtMs, {
                    ...
                    processing_reaped: processingWatchdog.status === 'fulfilled' ? Number(processingWatchdog.value ?.reaped || 0) : 0,
                    pending_resolved: pendingWatchdog.status === 'fulfilled' ? Number(pendingWatchdog.value ?.resolved || 0) : 0
                });
                ...
                Promise.resolve()
                    .then(async() => {
                        const botConfig = await getBinanceBotConfig(db).catch(() => null);
                        return warmMarginLeverageCache(
                            ACTIVE_SYMBOLS.slice(0, PREALERT_MARGIN_LEVERAGE_WARM_SYMBOLS), {
                                leverage: Number(botConfig ?.default_leverage || process.env.BINANCE_DEFAULT_LEVERAGE || 3),
                                marginType: String(botConfig ?.margin_type || 'ISOLATED').toUpperCase()
                            }
                        );
                    })
                    .then((summary) => {
                        console.log('[CRON] margin leverage warmup summary', {
                            symbols_total: Number(summary ?.symbols_total || 0),
                    ...
                    .catch((err) => {
                        console.warn('[CRON] margin leverage warmup failed', err ?.message || err);
                    });
...
async function runBinanceManagerCycle() {
    try {
        await runActivePositionSafetyControl();
        console.log('[CRON] impulse safety control completed');
    } catch (err) {
        console.warn('[CRON] impulse safety control failed', err.message);
    }
    try {
...
function parseVerificationDateLike(value) {
...
function resolveVerificationCandidateTimestamp(data = {}) {
    return (
        parseVerificationDateLike(data.signal_emitted_at) ||
        parseVerificationDateLike(data.signal_ready_at) ||
        parseVerificationDateLike(data.signal_created_at) ||
        parseVerificationDateLike(data.created_at) ||
        parseVerificationDateLike(data.timestamp) ||
        null
    );
}

function isShadowVerificationCandidate(data = {}) {
    return (
        data.signal_emitted === true ||
        (
            data.signal_emitted === false &&
            String(data.suppression_reason || '').toLowerCase() === 'low_confidence'
        )
    );
}

function hasIncompleteShadowValidation(data = {}) {
    if (!data.shadow_validation) return true;
    const status = String(data.shadow_validation.status || '').toLowerCase();
    return status === 'pending' || status === 'partial' || status === 'error';
}

function getShadowValidationStatus(data = {}) {
    if (!data.shadow_validation) return 'missing';
    return String(data.shadow_validation.status || '').toLowerCase() || 'missing';
}

function getShadowSelectionPriority(data = {}) {
    if (!data.shadow_validation) return 0;
    const status = getShadowValidationStatus(data);
...
function buildVerificationCandidatesForCycle(docs = [], cutoff) {
    let candidateShadow = 0;
    let skippedByAge = 0;
    let skippedByStatus = 0;
    let skippedBySuppressedVerification = 0;
    let skippedByExistingCompletedShadow = 0;
    ...
    for (const doc of docs) {
        ...
        const isShadowCandidate = isShadowVerificationCandidate(data);
        const shadowValidationStatus = getShadowValidationStatus(data);
        const shadowValidationIncomplete = isShadowCandidate && hasIncompleteShadowValidation(data);
        const shadowTimestamp = resolveVerificationCandidateTimestamp(data);
        const shadowMs = shadowTimestamp ?.getTime ?.() || 0;
        ...
        const shouldProcessLegacySuppressed = isSuppressed && !hasSuppressedVerification;
        const createdAt = data.created_at || data.timestamp;
        const createdMs = parseVerificationDateLike(createdAt) ?.getTime ?.() || 0;
        const sortMs = shadowMs || createdMs || 0;

        if (isShadowCandidate) {
            candidateShadow += 1;
            if (shadowValidationStatus === 'completed') {
                skippedByExistingCompletedShadow += 1;
                skippedByStatus += 1;
                continue;
            }
            if (!shadowValidationIncomplete) {
                skippedByStatus += 1;
                continue;
            }
            if (!shadowMs || shadowMs > cutoff) {
                skippedByAge += 1;
                continue;
            }
            eligibleCandidates.push({
                ...
                data,
                isSuppressed,
                priority: getShadowSelectionPriority(data),
                sortMs
            });
            continue;
        }
        ...
        if (shouldProcessLegacySuppressed) {
            eligibleCandidates.push({
                doc,
                data,
                isSuppressed,
        ...
        if (isSuppressed && hasSuppressedVerification) {
            skippedBySuppressedVerification += 1;
            continue;
        }
    ...
    return {
        ...
        skippedByAge,
        skippedByStatus,
        skippedBySuppressedVerification,
        skippedByExistingCompletedShadow,
        eligibleCandidates,
        selectedCandidates: eligibleCandidates.slice(0, MAX_SHADOW_EVALUATIONS_PER_CYCLE)
    };
}
...
async function runVerificationCycle() {
    ...
    let skippedByStatus = 0;
    let skippedBySuppressedVerification = 0;
    let shadowValidationWritten = 0;
    let shadowStatusPending = 0;
    let shadowStatusPartial = 0;
    let shadowStatusCompleted = 0;
    let shadowStatusError = 0;
    let skippedByMissingPrice = 0;
    let skippedByMissingSignalTimestamp = 0;
    let skippedByTimeBudget = 0;
    let skippedByExistingCompletedShadow = 0;
    let totalDocumentsRead = 0;
    let totalSelectedForCycle = 0;
    let pendingSnapshot = { docs: [], size: 0 };
    let suppressedSnapshot = { docs: [], size: 0 };
    let shadowRecentSnapshot = { docs: [], size: 0 };
    ...
    try {
        ...
        try {
            ...
            suppressedSnapshot = await db
                ...
                .limit(50)
                .get();
            shadowRecentSnapshot = await db
                .collection('velas_predicciones')
                .orderBy('updated_at', 'desc')
                .limit(SHADOW_VERIFICATION_QUERY_LIMIT)
                .get();
        } catch (err) {
        ...
        for (const doc of[...pendingSnapshot.docs, ...suppressedSnapshot.docs, ...shadowRecentSnapshot.docs]) {
            if (!docsById.has(doc.id)) {
                docsById.set(doc.id, doc);
            }
        }
        const docs = Array.from(docsById.values());
        totalDocumentsRead = docs.length;

        const candidateSelection = buildVerificationCandidatesForCycle(docs, cutoff);
        candidateShadow = candidateSelection.candidateShadow;
        skippedByAge = candidateSelection.skippedByAge;
        skippedByStatus = candidateSelection.skippedByStatus;
        skippedBySuppressedVerification = candidateSelection.skippedBySuppressedVerification;
        skippedByExistingCompletedShadow = candidateSelection.skippedByExistingCompletedShadow;
        totalSelectedForCycle = candidateSelection.selectedCandidates.length;
        skipped += Math.max(0, candidateSelection.eligibleCandidates.length - totalSelectedForCycle);

        console.log('[CRON] runVerificationCycle shadow diagnostic', {
            total_documents_read: totalDocumentsRead,
            total_candidates_shadow: candidateShadow,
            total_candidates_selected_for_this_cycle: totalSelectedForCycle,
            max_shadow_evaluations_per_cycle: MAX_SHADOW_EVALUATIONS_PER_CYCLE,
            max_runtime_ms: MAX_VERIFICATION_RUNTIME_MS,
            shadow_fetch_concurrency: MAX_SHADOW_FETCH_CONCURRENCY,
            total_skipped_by_age_insufficient: skippedByAge,
            total_skipped_by_status: skippedByStatus,
            total_skipped_by_existing_completed_shadow: skippedByExistingCompletedShadow,
            total_skipped_by_existing_suppressed_verification: skippedBySuppressedVerification
        });

        for (let index = 0; index < candidateSelection.selectedCandidates.length; index += 1) {
            if (Date.now() - cycleStartedAtMs > MAX_VERIFICATION_RUNTIME_MS) {
                skippedByTimeBudget += candidateSelection.selectedCandidates.length - index;
                break;
            }

            const candidate = candidateSelection.selectedCandidates[index];
            const doc = candidate.doc;
            const candidateData = doc.data() || {};
            const documentLog = {
                document_id: doc.id,
                symbol: candidateData.simbolo_normalizado || candidateData.simbolo || candidateData.symbol || null,
                signal_emitted: candidateData.signal_emitted,
                suppression_reason: candidateData.suppression_reason || null,
                initial_status: candidateData.status || null
            };

            try {
                const result = await verificarPrediccionVelas(doc.id);
                verified += 1;
                if (candidate.isSuppressed) {
                    suppressedBackfilled += 1;
                }
                const shadowValidation = result ?.shadow_validation || null;
                const firestoreUpdateSuccess = result ?.firestore_update_success === true;
                if (shadowValidation) {
                    if (firestoreUpdateSuccess) {
                        shadowValidationWritten += 1;
                    }
                    if (shadowValidation.status === 'pending') shadowStatusPending += 1;
                    if (shadowValidation.status === 'partial') shadowStatusPartial += 1;
                    if (shadowValidation.status === 'completed') shadowStatusCompleted += 1;
                    if (shadowValidation.status === 'error') shadowStatusError += 1;
                    const notes = Array.isArray(shadowValidation.notes) ? shadowValidation.notes : [];
                    if (notes.includes('missing_entry_price')) skippedByMissingPrice += 1;
                    if (notes.includes('missing_signal_timestamp')) skippedByMissingSignalTimestamp += 1;
                }

                console.log('[CRON] runVerificationCycle document processed', {
                    ...documentLog,
                    ...
                    firestore_update_attempted: result ?.firestore_update_attempted === true,
                    firestore_update_success: firestoreUpdateSuccess,
                    firestore_update_error: result ?.firestore_update_error || null
                });
            } catch (err) {
                failed += 1;
                console.error('[CRON] runVerificationCycle document failed', {
                    ...documentLog,
                    shadow_validation_status: null,
                    horizons_length: 0,
                    firestore_update_attempted: true,
                    firestore_update_success: false,
                    firestore_update_error: err.message
                });
        ...
        console.log('[CRON] runVerificationCycle finished', {
            ...
            shadow_recent_total: shadowRecentSnapshot.size,
            total_candidates_shadow: candidateShadow,
            total_selected_for_cycle: totalSelectedForCycle,
            total_evaluated: verified,
            total_written_with_shadow_validation: shadowValidationWritten,
            total_skipped_by_time_budget: skippedByTimeBudget,
            total_skipped_by_existing_completed_shadow: skippedByExistingCompletedShadow,
            total_skipped_by_age_insufficient: skippedByAge,
            total_skipped_by_status: skippedByStatus,
            total_skipped_by_existing_suppressed_verification: skippedBySuppressedVerification,
            total_skipped_by_missing_price: skippedByMissingPrice,
            total_skipped_by_missing_signal_timestamp: skippedByMissingSignalTimestamp,
            total_shadow_pending: shadowStatusPending,
            total_shadow_partial: shadowStatusPartial,
            total_shadow_completed: shadowStatusCompleted,
            total_shadow_error: shadowStatusError,
            pending_resolved: pendingResolved,
            suppressed_backfilled: suppressedBackfilled,
            total_skipped: skipped,
            total_failed: failed,
            elapsed_ms: Date.now() - cycleStartedAtMs
        });
    }
}
        ...
        for (const symbol of symbols) {
            try {
                ...
                if (result.impulseDetected) {
                    ...
                    console.log(`[IMPULSE_CYCLE] âœ“ IMPULSE DETECTED: ${symbol}`, {
                        direction: result.direction,
                    ...
                    console.log(`[IMPULSE_CYCLE] âœ— ${symbol}: ${result.reason || 'No impulse'}`);
                }
            } catch (err) {
