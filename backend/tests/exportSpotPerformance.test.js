...
const trades = [
  {
    ...
    net_pnl_pct: 20, fully_closed: true, close_source: 'BINANCE_ORDER'
  },
  {
    ...
    net_pnl_pct: -10, fully_closed: true, close_source: 'BINANCE_ORDER'
  },
  {
    id: 'external', symbol: 'CCCUSDT', pnl_verified: true, net_pnl_usdt: 99,
    total_fee_usdt: 10, allocated_capital_usdt: 10, closed_at: '2026-08-28T12:00:00Z',
    close_source: 'BINANCE_CONVERT', performance_excluded: true,
    performance_exclusion_reason: 'EXTERNAL_CONVERSION'
  },
  {
    id: 'unverified', symbol: 'DDDUSDT', pnl_verified: false, net_pnl_usdt: 99,
    total_fee_usdt: 10, allocated_capital_usdt: 10, closed_at: '2026-08-28T12:00:00Z'
  }
];

const summary = summarizeTrades(trades, now);
assert.strictEqual(summary.verified_bot_trades, 2);
...
assert.strictEqual(safe.performance_classification, 'BOT_EXECUTION');
assert.strictEqual(safe.performance_exclusion_reason, null);

const excluded = sanitizeTrade(trades[2]);
assert.strictEqual(excluded.performance_classification, 'EXCLUDED');
assert.strictEqual(excluded.performance_exclusion_reason, 'EXTERNAL_CONVERSION');
