...
/**
 * Diagnóstico de candidatos shadow con no_exit_condition_met
 */

const db = require('./backend/firebase-admin-config');
const { parseDateLike, toNumber, normalizeSymbol, SHADOW_MAX_HOLD_MINUTES, SHADOW_EVENT_TIMEOUT_RATIO } = require('./backend/lib/shadowEdgeSamplerDiagnostic');
...
function toNumberLocal(value, fallback = null) {
    const num = Number(value);
    return Number.isFinite(num) ? num : fallback;
}

async function loadCandlesForSymbolLocal(symbol, cache) {
    if (cache.has(symbol)) {
...
async function inspectNoExitCandidates() {
    ...
    try {
        ...
        for (const candidate of candidatesData) {
            ...
            const entryAt = parseDateLikeLocal(candidate.simulated_entry_at);
            if (!entryAt) continue;
            ...
            try {
                const candles = await loadCandlesForSymbolLocal(candidate.symbol, candleCache);
                const futureCandles = candles.filter(candle =>
                    toNumberLocal(candle ? .timestamp, 0) > entryAt.getTime()
                );

                // Calcular timeouts
                const maxHoldMs = (SHADOW_MAX_HOLD_MINUTES || 10) * 60 * 1000;
                const eventTimeoutMs = Math.round(maxHoldMs * (SHADOW_EVENT_TIMEOUT_RATIO || 0.8));

                // Calcular tiempo disponible
                const lastCandle = futureCandles[futureCandles.length - 1];
                const lastCandleAt = lastCandle ? parseDateLikeLocal(lastCandle.timestamp) : null;
                ...
                console.log(`   normalized_symbol: ${normalizeSymbolLocal(candidate.symbol)}`);
        ...
        if (resultsSnap.empty) {
            ...
            results.forEach(r => {
                const sym = normalizeSymbolLocal(r.symbol);
            ...
            const btcResults = results.filter(r => normalizeSymbolLocal(r.symbol) === 'BTCUSDT');
            const solResults = results.filter(r => normalizeSymbolLocal(r.symbol) === 'SOLUSDT');

            console.log(`   BTCUSDT count: ${btcResults.length} (debería ser > 0 para contar)`);
            console.log(`   SOLUSDT count: ${solResults.length} (debería ser > 0 para contar)`);

            if (btcResults.length === 0 && solResults.length === 0) {
                console.log('   PROBLEMA: Los resultados existentes no son de BTCUSDT ni SOLUSDT');
                console.log('   Símbolos encontrados:', [...new Set(results.map(r => normalizeSymbolLocal(r.symbol)))]);
            }
        }
